...
fn executeDrawCommandsNonIndexed(self: *Engine, cmd: vk.CommandBuffer, pass: *MeshPass, obj_data_set: vk.DescriptorSet, dyn_offsets: []const u32, global_set: vk.DescriptorSet) !void {
    ...
    var fooker = (try pass.compacted_instance_buffer.mapMemory(self.gc.vma))[0..pass.flat_batches.items.len];
    defer pass.compacted_instance_buffer.unmapMemory(self.gc.vma);
    ...
    for (pass.flat_batches.items) |rb, i| {
        ...
        // if (fooker[i] < pass.flat_batches.items.len) {
        var obj_id: u32 = std.math.maxInt(u32);
        for (fooker) |fook, j| {
            if (fook == obj.original.handle) {
                if (fook < pass.flat_batches.items.len)
                    obj_id = @intCast(u32, j);
                break;
            }
        }

        if (obj_id != std.math.maxInt(u32)) {
            self.gc.vkd.cmdDrawIndexed(cmd, @intCast(u32, draw_mesh.indices.len), 1, 0, 0, obj_id);
            // std.debug.print("obj_id: {}, obj.original.handle: {}\n", .{ obj_id, obj.original.handle });
        }
        // }
    }
}
